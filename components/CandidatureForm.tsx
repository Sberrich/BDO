...
export function CandidatureForm() {
  ...
  return (
    <form ref={formRef} onSubmit={onSubmit} onChange={(e) => persist(e.currentTarget)} className="space-y-6">
      <div>
        <ol className="flex flex-wrap gap-x-5 gap-y-2 text-[0.8125rem]">
          {steps.map((label, i) => (
            <li key={label} className={`flex items-center gap-2 ${i === step ? "font-bold text-ink" : i < step ? "font-semibold text-ink" : "text-muted"}`}>
              <span
        ...
        <div className="mt-3 h-1.5 overflow-hidden rounded-sm bg-cream">
          <div className="h-full bg-blue transition-all" style={{ width: `${((step + 1) / steps.length) * 100}%` }} />
        </div>
        <p className="mt-2 text-xs text-muted">Brouillon conservé sur cet appareil. Vous pouvez reprendre plus tard.</p>
      </div>
      ...
      <fieldset className="grid gap-4 sm:grid-cols-2" hidden={step !== 0}>
        <label className="block text-sm font-semibold">
          Nom et prénom *<input className={fieldClass} required={step === 0} name="nom" autoComplete="name" />
        </label>
        <label className="block text-sm font-semibold">
          Fonction exacte *<input className={fieldClass} required={step === 0} name="fonction" />
        </label>
        <label className="block text-sm font-semibold">
          Entreprise *<input className={fieldClass} required={step === 0} name="entreprise" />
        </label>
        <label className="block text-sm font-semibold">
          Secteur *
        ...
        <label className="block text-sm font-semibold">
          E-mail professionnel *<input className={fieldClass} required={step === 0} type="email" name="email" />
        </label>
        <label className="block text-sm font-semibold">
          Téléphone *<input className={fieldClass} required={step === 0} type="tel" name="telephone" />
        </label>
      </fieldset>

      <fieldset className="grid gap-4 sm:grid-cols-2" hidden={step !== 1}>
        <label className="block text-sm font-semibold">
          Années d’expérience en finance *<input className={fieldClass} required={step === 1} type="number" min={5} name="experience" />
        </label>
        <label className="block text-sm font-semibold">
          Dernier diplôme *<input className={fieldClass} required={step === 1} name="diplome" />
        </label>
        <label className="block text-sm font-semibold">
          Rattachement hiérarchique *<input className={fieldClass} required={step === 1} name="rattachement" />
        </label>
        <label className="block text-sm font-semibold">
          Équipe encadrée *
      ...
      <fieldset className="space-y-4" hidden={step !== 2}>
        <p className="text-sm text-muted">Ces réponses servent à constituer les sous-groupes dès la conférence inaugurale.</p>
        <label className="block text-sm font-semibold">
          Le chantier de transformation visé *<textarea className={`${fieldClass} min-h-32`} required={step === 2} maxLength={800} name="projet" />
        </label>
        <label className="block text-sm font-semibold">
          Ce que votre entreprise attend *<textarea className={`${fieldClass} min-h-32`} required={step === 2} maxLength={800} name="attente" />
        </label>
      </fieldset>
      ...
      <fieldset className="space-y-4" hidden={step !== 3}>
        <label className="block text-sm font-semibold">
          Mode de financement *
          <select className={fieldClass} required={step === 3} name="financement">
            <option value="">Choisissez…</option>
        ...
        <label className="block text-sm font-semibold">
          Contact RH (facultatif)<input className={fieldClass} name="contact_rh" />
        </label>
        <label className="block text-sm font-semibold">
          Disponibilité *
          <select className={fieldClass} required={step === 3} name="disponibilite">
            <option value="">Choisissez…</option>
            <option value="oui">Oui, sur l’ensemble du calendrier</option>
            <option value="partiel">Oui, avec une ou deux indisponibilités</option>
            <option value="non">Non, je souhaite en discuter</option>
          </select>
        </label>
        <label className="flex items-start gap-3 text-sm">
          <input type="checkbox" required={step === 3} name="consentement" value="oui" className="mt-1" />
          <span>J’accepte que ce dossier soit traité par le Groupe ISCAE et BDO Maroc pour instruire ma candidature. *</span>
        </label>
        <label className="flex items-start gap-3 text-sm">
          <input type="checkbox" name="prospection" value="oui" className="mt-1" />
      ...
      <div className="flex flex-wrap justify-between gap-3">
        ...
        <Button type="submit" disabled={sending}>
          {sending ? "Envoi…" : step === steps.length - 1 ? "Envoyer ma candidature" : "Étape suivante"}
        </Button>
      </div>
      {status && <p className="text-sm text-red">{status}</p>}
      <p className="text-xs text-muted">Clôture des candidatures le {plain(data.site.admission.dateLimite)}.</p>
    </form>
  );
}
